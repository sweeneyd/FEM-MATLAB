...
for i=1:length(elements)
    ...
    h = verts(nodes(2)) - verts(nodes(1));
    
    midpt1 = h/3 + verts(nodes(1));
    midpt2 = 2*h/3 + verts(nodes(1));
    
    f1 = @(xi) -9/2*(xi-midpt1).*(xi-midpt2).*(xi-verts(nodes(2)))./h^3;
    f2 = @(xi) 27/2*(xi-verts(nodes(1))).*(xi-verts(nodes(2))).*(xi-midpt2)./h^3;
    f3 = @(xi) -27/2*(xi-verts(nodes(1))).*(xi-verts(nodes(2))).*(xi-midpt1)./h^3;
    f4 = @(xi) 9/2*(xi-midpt1).*(xi-midpt2).*(xi-verts(nodes(1)))./h^3;
